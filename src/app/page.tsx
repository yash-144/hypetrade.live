...
// FONT SETUP: Initialize Inter font
const inter = Inter({ subsets: ['latin'], variable: '--font-inter' });

// --- NEW GLITCHY HERO SECTION COMPONENT ---
const GlitchHero = () => {
    const xWords = ['Hype', 'Buzz', 'Clout', 'Trend', 'Fame', 'Heat'];
    const yWords = ['Momentum', 'Spikes', 'Noise', 'Energy', 'Growth'];
    ...
    useEffect(() => {
        const xInterval = setInterval(() => {
            setXIndex((prevIndex) => (prevIndex + 1) % xWords.length);
        }, 2500);

        const yInterval = setInterval(() => {
          setYIndex((prevIndex) => (prevIndex + 1) % yWords.length);
        }, 2500);
            ...
            <div className="absolute inset-0 z-0">
                ...
                ></div>
                 <div 
                    ...
                    style={{'--grid-size': '240px', '--grid-color': 'rgba(255, 50, 68, 0.1)'} as React.CSSProperties}
                ></div>
            </div>
                ...
                <h1 className="text-5xl md:text-7xl font-extrabold text-white leading-tight tracking-tighter">
                    ...
                    <AnimatePresence mode="wait">
                        <motion.span
                            ...
                            animate="animate"
                            exit="exit"
                            className="text-red-500 inline-block glitch-text"
                        >
                            {xWords[xIndex]}
                        </motion.span>
                    </AnimatePresence>
                    ...
                    Track Viral{' '}
                     <AnimatePresence mode="wait">
                        <motion.span
                            ...
                            animate="animate"
                            exit="exit"
                            className="text-orange-400 inline-block glitch-text"
                        >
                            {yWords[yIndex]}
                        </motion.span>
                    </AnimatePresence>
    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-black/30 backdrop-blur-lg border-b border-white/10">
      <div className="container mx-auto px-6 md:px-12 py-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-4">
          <Link href="/login" className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-5 rounded-full text-sm transition-transform transform hover:scale-105">
            Login
          </Link>
        </div>
      ...
      <div className={`md:hidden absolute top-full left-0 w-full bg-black/95 backdrop-blur-md transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
        <nav className="flex flex-col items-center gap-6 py-8">
          ...
          <Link href="/login" className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-full text-lg transition-transform transform hover:scale-105" onClick={() => setIsMenuOpen(false)}>
            Join Waitlist
          </Link>
        </nav>
      </div>
...
const FeatureCard = ({ icon, title, children }: { icon: React.ReactNode, title: string, children: React.ReactNode }) => (
  <div className="relative p-px rounded-lg bg-gradient-to-b from-red-500/50 to-gray-800/10 shadow-lg hover:shadow-2xl transition-shadow duration-300">
    <div className="absolute inset-0 rounded-lg bg-gradient-to-b from-red-500/20 to-gray-900/10 opacity-20 hover:opacity-100 transition-opacity duration-500 animate-pulse-slow"></div>
...
export default function HypeTradeLandingPage() {
  ...
  return (
    <div className={`${inter.variable} font-sans bg-black text-gray-200`}>
      <style jsx global>{`
        .bg-grid-pattern {
        ...
        /* UPDATED: CSS for the glitch text effect with new red/orange colors */
        .glitch-text {
            filter: url(#glitch-filter);
            animation: glitch-anim 4s linear infinite;
        }
        @keyframes glitch-anim {
            0% { text-shadow: -2px -2px 0 #ef4444, 2px 2px 0 #fb923c; }
            25% { text-shadow: 2px 2px 0 #ef4444, -2px -2px 0 #fb923c; }
            50% { text-shadow: 2px -2px 0 #ef4444, -2px 2px 0 #fb923c; }
            75% { text-shadow: -2px 2px 0 #ef4444, 2px -2px 0 #fb923c; }
            100% { text-shadow: -2px -2px 0 #ef4444, 2px 2px 0 #fb923c; }
        }
      `}</style>
      <Header />
        ...
        <main className="container mx-auto px-6 py-12 md:px-12">
            ...
            <section id="demo" ref={howItWorksRef} className={`py-20 md:py-28 text-center scroll-mt-20 ${animationClasses(howItWorksIsVisible)}`}>
              ...
              <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                <div className="flex flex-col items-center">
                  <div className="bg-red-600/10 p-4 rounded-full mb-4 border border-red-500/20">
                ...
                <div className="flex flex-col items-center">
                  <div className="bg-red-600/10 p-4 rounded-full mb-4 border border-red-500/20">
                    <BarChart className="text-red-400" size={32} />
                  </div>
            ...
            <section id="features" ref={whyRef} className={`py-20 md:py-28 scroll-mt-20 ${animationClasses(whyIsVisible)}`}>
              ...
              <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                <FeatureCard icon={<BarChart className="text-red-400" size={24} />} title="Simulated Creator Market">
                  Watch virtual prices rise or fall based on real-time social metrics and app sentiment.
                </FeatureCard>
                <FeatureCard icon={<Gamepad2 className="text-red-400" size={24} />} title="Skill-Based Strategy Game">
                  Make smart trades based on timing, trends, and instinct — all in a safe, virtual economy.
                </FeatureCard>
                <FeatureCard icon={<ShieldCheck className="text-red-400" size={24} />} title="Zero Risk Environment">
                  No real money. No crypto. Just a data-driven entertainment experience.
                </FeatureCard>
                <FeatureCard icon={<Award className="text-red-400" size={24} />} title="Leaderboard Bragging Rights">
            ...
            <hr className="border-gray-800/50" />
            <section id="waitlist" ref={ctaRef} className={`py-20 md:py-28 text-center bg-red-900/20 rounded-xl my-16 scroll-mt-20 ${animationClasses(ctaIsVisible)}`}>
                ...
                <div className="mt-8 flex justify-center">
                     <a href="#waitlist-form" className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-full text-lg transition-transform transform hover:scale-105">
                        Join the Waitlist
                    </a>
                </div>
            </section>
          ...
          <div className="container mx-auto px-6 py-8 md:px-12">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="text-center md:text-left">
                      ...
                      <p className="text-gray-400 text-sm mt-1">Contact: <a href="mailto:hypetrade.preview@gmail.com" className="hover:text-red-400">hypetrade.preview@gmail.com</a></p>
                  </div>
